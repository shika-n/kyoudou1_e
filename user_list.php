...
while ($record = $sql_res->fetch()) {
    ...
    $user = htmlspecialchars($record['user'], ENT_QUOTES, 'UTF-8');
    $nickname = htmlspecialchars($record['nickname'], ENT_QUOTES, 'UTF-8');

    $user_list .= <<< ___EOF___
        <li class="flex items-center mb-4">
            <img src="$icon" alt="icon" class="w-12 h-12 rounded-full mr-4">
...
$content = <<< ___EOF___
    ...
    <!--ユーザー情報取得・表示-->
    <ul class="border-8 border-slate-300 w-[960px] rounded-md h-96 my-2 mx-2 p-2 text-xl overflow-auto">
        $user_list
    </ul>
